...
class AnchorLoader(mx.io.DataIter):
    ...
    def __init__(self, feat_sym, roidb, cfg, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
        ...
        if config.TRAIN.END2END:
            self.data_name = ['data', 'ref_data', 'im_info', 'ref_im_info', 'gt_boxes', 'ref_gt_boxes']
        else:
            self.data_name = ['data']
        self.label_name = ['label', 'ref_label', 'bbox_target', 'ref_bbox_target', 'bbox_weight', 'ref_bbox_weight']
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        ref_label = assign_anchor(feat_shape[0], np.zeros((0, 5)), im_info, self.cfg,
                              self.feat_stride, self.anchor_scales, self.anchor_ratios, self.allowed_border,
                              self.normalize_target, self.bbox_mean, self.bbox_std)
        for k, v in ref_label.items():
            label['ref_'+k] = v
    ...
    def parfetch(self, iroidb):
        ...
        # assign anchor for label
        _label = label
        label = assign_anchor(feat_shape, _label['gt_boxes'], data['im_info'], self.cfg,
                              self.feat_stride, self.anchor_scales,
                              self.anchor_ratios, self.allowed_border,
                              self.normalize_target, self.bbox_mean, self.bbox_std)

        ref_label = assign_anchor(feat_shape, _label['ref_gt_boxes'], data['ref_im_info'], self.cfg,
                              self.feat_stride, self.anchor_scales,
                              self.anchor_ratios, self.allowed_border,
                              self.normalize_target, self.bbox_mean, self.bbox_std)
        for k, v in ref_label.items():
